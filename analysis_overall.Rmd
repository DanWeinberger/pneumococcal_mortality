...
p2a <- death_plot_fun(disease='ipd',year,month, agec) 
p3a <- p2a$plot1 + facet_wrap(~agec, scales='free', nrow=1) +ggtitle("IPD")
...
```{r}
p2b <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',year,month, agec) 
p3b <- p2b$plot1 + facet_wrap(~agec, scales='free', nrow=1) +ggtitle("Non-invasive pneumococcal pneumonia")
...
```{r, fig.width=12, fig.height=6}
p3a <- death_plot_fun(disease='ipd',year,month, race_recode_new) 
p4a <- p3a$plot1+ facet_wrap(~race_recode_new, scales='free', nrow = 1) + ggtitle('IPD')

p3b <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',year,month, race_recode_new) 
p4b <- p3b$plot1+ facet_wrap(~race_recode_new, scales='free', nrow = 1) + ggtitle('Non-invasive pneumococcal pneum.')

grid.arrange(p4a, p4b,nrow=2)
```

```{r,fig.width=unit(15,"cm"), fig.height=unit(7,"cm")}
p3a <- death_plot_fun(disease='ipd',year,month, race_recode_new, agec) 

p3a$ts$date <- as.Date(paste(p3a$ts$year, p3a$ts$month, '01',sep='-') , '%Y-%m-%d' )

p3a$ts$year <- as.numeric(as.character(p3a$ts$year))

p3a.ds <- merge(p3a$ts, pop2, by=c('agec','race_recode_new', 'date', 'year')) %>%
...
ggplot(p3a.ds, aes(x=agec, y=log(inc), group=race_short,col=race_short, linetype=race_short)) +
  geom_line()+
  theme_classic()+
  ggtitle('Rate of IPD death by age and race/ethnicity')+
...
### Rate of Death by age and race/ethnicity

```{r}
###

p3b <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',year,month, race_recode_new, agec) 

p3b$ts$date <- as.Date(paste(p3b$ts$year, p3b$ts$month, '01',sep='-') , '%Y-%m-%d' )

p3b$ts$year <- as.numeric(as.character(p3b$ts$year))

p3b.ds <- merge(p3b$ts, pop2, by=c('agec','race_recode_new', 'date', 'year')) %>%
  filter(year<2020) %>% #Pre-padnemic rates
  group_by(race_recode_new, agec) %>%
  summarize(N_deaths=sum(N_deaths), pop=mean(pop.interpol))%>%
  ungroup() %>%
...
ggplot(p3b.ds, aes(x=agec, y=log(inc), group=race_short,col=race_short, linetype=race_short)) +
  geom_line()+
  theme_classic()+
  ggtitle('Rate of pneumococcal pneum. death by age and race/ethnicity')+
   theme(axis.text.x = element_text(angle = 45, vjust = 0.9, hjust=1)) +
  ylab('log(Deaths/100,000)') +
  xlab('') +
    geom_dl(aes(label = race_short), method = "first.qp") +
...
ipd1 <- death_plot_fun(disease='ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',year,month) 
...
pneumococcal_pneum_no_ipd <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',year,month) 

pneumococcal_pneum_no_ipd$ts.plot

pneumococcal_pneum_no_ipd$rr.plot
...
p3 <- death_plot_fun(disease='ipd',year,month, race_recode_new) 
