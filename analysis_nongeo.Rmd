---
title: "Analysis of rsv deaths in the US"
output:
...
```{r}
rsv.deaths <-  open_dataset("/Volumes/home/HCUPdata-CC0941-MEDSPH/NCHS_mortality_public/parquet3", format = "parquet")  %>%
  filter(grepl("J121|J210|J205|B974", all_icd)) %>% 
 collect() %>%
  mutate(rsv_pneumonia = if_else(grepl("J121", all_icd),1,0),
         rsv_acute_bronchiolitis = if_else(grepl("J210", all_icd),1,0),
         rsv_acute_bronchitis = if_else(grepl("J205", all_icd), 1, 0),
         rsv_others = if_else(grepl("B974", all_icd), 1,0),
