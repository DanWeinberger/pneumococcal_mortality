...
p4
```
**to check: why is rate in Hispanic so low?
...
```{r}
mydata1 <- read.csv('./Data/Wonder/Provisional Mortality Statistics, 2018 through Last Month 2023_07_28.txt', header=T, sep="\t")
...
p1 <- ggplot(mydata2, aes(x=month, y=N_deaths, group=year, color=year)) +
  geom_line()+
  theme_classic()+
  ylim(0, NA) +
  facet_wrap(~cause)
...
ds1 <- mydata2 %>%
  mutate(date=as.Date(paste(year, month, '01', sep='-'))) %>%
          ...
          group_by(date) %>%
          summarize(N_deaths=sum(N_deaths)) %>%
          rename(N_deaths_wonder=N_deaths) %>%
