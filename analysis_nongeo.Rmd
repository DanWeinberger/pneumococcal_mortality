...
```{r}
p1_ipd <-  death_plot_fun(disease='ipd',year,month )

p1_ipd$plot1 +
  ggtitle('IPD')


p1_pneumococcal_pneum <- death_plot_fun(disease='pneumococcal_pneum',year,month, )
p1_pneumococcal_pneum$plot1 +
  ggtitle('Pneumococcal pneumonia')

p1_pneumococcal_pneum_no_ipd <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',year,month )
...
p1 <- death_plot_fun(disease='pneumococcal',year,month, a403 )
p1$plot1 +
  facet_wrap(~a403) +
    ggtitle('A40.3 vs other pneumococcal codes')


p1 <- death_plot_fun(disease='pneumococcal',year,month, j154 )
p1$plot1 +
  facet_wrap(~j154)

p1 <- death_plot_fun(disease='pneumococcal',year,month, j181 )
p1$plot1 +
  facet_wrap(~j181)

p1.pneumococcal <- death_plot_fun(disease='pneumococcal',year,month ) 

p1.pneumococcal$plot1
```

```{r, fig.width=12, fig.height=3}
p2 <- death_plot_fun(disease='pneumococcal',year,month, agec) 
p2$plot1 + facet_wrap(~agec, scales='free', nrow=1)
```

By syndrome and age (note to check--if someone has J12-J18 + other pneumo code, it should count as pneumococcal pneumonia)
```{r, fig.height=5, fig.width=12}
p2a <- death_plot_fun(disease='pneumococcal',year,month, agec, pneumococcal_pneum) 
p2a$plot1 + facet_grid(pneumococcal_pneum~agec, scales='free')
```
...
p3.ds <- merge(p3$ts, pop2, by=c('agec','race_recode_new', 'date', 'year')) %>%
...
ggplot(p3.ds, aes(x=agec, y=log(inc), group=race_short,col=race_short, linetype=race_short)) +
  geom_line()+
  theme_classic()+
  ...
  xlab('') +
    geom_dl(aes(label = race_short), method = "first.qp") +
  ...
  xlab('') +
   ...
   # Allow labels to bleed past the canvas boundaries
    ...
    theme(legend.position = 'none',
          plot.margin = margin(0.1, 2.6, 0.1, 0.1, "cm")) 
```
Sex and age--need to redo this to get pop size
```{r}
...
# p3$ts$year <- as.numeric(as.character(p3$ts$year))
# 
# p3.ds <- merge(p3$ts, pop2.sex, by=c('agec','sex', 'date', 'year')) %>%
#   filter(year<2020) %>% #Pre-padnemic rates
#   group_by(sex, agec) %>%
#   summarize(N_deaths=sum(N_deaths), pop=mean(pop.interpol))%>%
#   ungroup() %>%
...
```{r}
ds1 <- p1_pneumococcal_pneum_no_ipd$ts %>%
  mutate(date=as.Date(paste(year, month, '01', sep='-')),
         qtr=as.factor(lubridate::quarter(date)),
         index= interval('2014-01-01', date) %/% months(1) ,
         sin12 = sin(2*pi*index/12),
...
p1_b <- ggplot(data=ds1, aes(x=date, y=N_deaths)) +
  geom_line() +
  theme_classic()+
    geom_ribbon(aes(ymin=pred_lcl, ymax=pred_ucl), alpha=0.2)+
    geom_line(aes(x=date, y=pred_median), col='red', lty=2, alpha=0.2)+
...
#Poisson
mod2 <- glm(N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6  +  
