...
table( pneumo.deaths.in$pneumococcal_pneum,pneumo.deaths.in$pneumococcal_pneum_no_ipd)
...
pneumo.deaths <- pneumo.deaths.in

pneumo.deaths$agec <- factor(pneumo.deaths$agec, c(1,2,3,4,5,6,7),c("Under 5 Years",'5-24 years', "25-44 years","45-64 years","65-74 years" ,"75-84 years","85 years and older"))
...
pneumo.deaths.adult <- pneumo.deaths %>%
...
table(pneumo.deaths$j13,pneumo.deaths$ipd )
...
#pop data from CDC WONDER
pop2020_2022<- read.delim('./Data/Single-Race Population Estimates 2020-2022 by State and Single-Year Age.txt') %>%
  rename(agey=Single.Year.Ages,
         year="Yearly.July.1st.Estimates" ) %>%
  dplyr::select(agey, year, Population) %>%
  filter(!is.na(year)) %>%
  filter(year>=2021)

pop2010_2020<- read.delim('./Data/Single-Race Population Estimates 2010-2020 by State and Single-Year Age.txt') %>%
  rename(agey=Single.Year.Ages,
         year="Yearly.July.1st.Estimates" ) %>%
  dplyr::select(agey, year, Population) %>%
  filter(!is.na(year))

pop_combined <- bind_rows(pop2010_2020, pop2020_2022)%>%
  mutate(agey = if_else(agey=='< 1 year', '0', agey),
         agey = gsub('years','',agey),
        agey = gsub('year','',agey),
         agey = gsub('+','',agey, fixed=T),
        agey=as.numeric(agey))


N_deaths <- pneumo.deaths.in %>%
  mutate(agey =floor(agey),
         date= as.Date(paste(year, month,'01', sep='-')),
         year=year(date),
         agey = if_else(agey>85,85,agey)) %>%
 # filter(ipd==1) %>%
  group_by(agey, year) %>%
  summarize(N=n()) %>%
  ungroup() %>%
  full_join(pop_combined, by=c('agey','year')) %>%
...
ggplot(N_deaths,aes(x=agey, y=inc)) +
  geom_line() +
  theme_minimal()+
  ylab('Deaths/100000') +
  xlab('Age (Years)')
...
```
Pandemic excess deaths by single year of age--INLA model of age effetc, pandemic effect and time
```{r}
N_deaths_yr <- pneumo.deaths.in %>%
  mutate(agey =floor(agey),
         date= as.Date(paste(year, month,'01', sep='-')),
         year=year(date),
         agey = if_else(agey>85,85,agey)) %>%
  ...
  mutate(N_deaths= if_else(is.na(N_deaths),0,N_deaths),
         ...
         agey3=agey/85,
         t=year,
         pandemic_period=if_else(year>=2020,1,0),
         log.offset=log(Population/100000)) %>%
  filter(year<=2021)


formula1 <- N_deaths ~  0 + #intercept
  f(agey,model = "ar1", hyper = list(theta1 = list(prior = "loggamma", param = c(3, 2))))+  #age-specific intercept
  f(agey2,constr=T,t,model = "ar1", hyper = list(theta1 = list(prior = "loggamma", param = c(3, 2)))) +   #age-varying effect for time
  f(agey3, constr=T,pandemic_period,model = "ar1", hyper = list(theta1 = list(prior = "loggamma", param = c(3, 2))))+  #age-varying effect for pandemic
  offset(log.offset)
...
inla.out <- inla(formula1, family = "nbinomial", data = N_deaths_yr,
...
p0.fitted <- ggplot(b1, aes(x=agey*85, y=log(pred1/Population*100000), group=t, col=t))+
  geom_line()+
  theme_classic()
p0.fitted
...
p1.age.inla <- ggplot(re.age, aes(x=ID*85, y=mean))+
  geom_line()+
  theme_classic()+
  geom_ribbon(data=re.age, aes(x=ID*85, ymin=`0.025quant`, ymax=`0.975quant`), alpha=0.2)
p1.age.inla

re.age.time <- inla.out$summary.random$agey2 %>%
  mutate()

p2.time.age.inla <- ggplot(re.age.time, aes(x=ID*85, y=exp(mean)))+
  geom_line()+
  theme_classic()+
  geom_ribbon(data=re.age.time, aes(x=ID*85, ymin=exp(`0.025quant`), ymax=exp(`0.975quant`)), alpha=0.2)+
...
p3.pandemic.age.inla <- ggplot(re.age.pandemic, aes(x=ID*85, y=exp(mean)))+
  geom_line()+
  theme_classic()+
  geom_ribbon(data=re.age.pandemic, aes(x=ID*85, ymin=exp(`0.025quant`), ymax=exp(`0.975quant`)), alpha=0.2) +
  geom_hline(yintercept = 1)
...
p1_pneumococcal <-  death_plot_fun(disease='pneumococcal',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',yearN,monthN )

p1_ipd <-  death_plot_fun(disease='ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',yearN,monthN )

p1_ipd$plot1 +
  ggtitle('IPD')


p1_pneumococcal_pneum <- death_plot_fun(disease='pneumococcal_pneum',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',yearN,monthN )
p1_pneumococcal_pneum$plot1 +
  ggtitle('Pneumococcal pneumonia')

p1_pneumococcal_pneum_no_ipd <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',yearN,monthN )

p1_pneumococcal_pneum_no_ipd$plot1 +
  ggtitle('Pneumococcal pneumonia (no ipd)')

```


```{r, fig.width=12, fig.height=3}
p2 <- death_plot_fun(disease='pneumococcal',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6 +agec',yearN,monthN, agec) 
p2$plot1 + facet_wrap(~agec, scales='free', nrow=1)
```

```{r, fig.width=12, fig.height=3}
p2a <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6 +agec',yearN,monthN, agec) 
p2a$plot1 + facet_wrap(~agec, scales='free', nrow=1)
```

```{r, fig.width=12, fig.height=3}
p2b <- death_plot_fun(disease='ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6 + agec',yearN,monthN, agec) 

p2b$plot1 + facet_wrap(~agec, scales='free', nrow=1)
```


## Alert
These plots suggest that we can't reliably break out IPD from non-invasive pneumococcal pneumonia. White/Black both show spikes during Delta wave, but one shows a spike in IPD only and the other shows a soike in NIPP, suggesting the coding isnt telling us what we think it is.

Note; big differences in age distribution. Hispanic skews much younger
```{r, fig.width=12, fig.height=3}
p3 <- death_plot_fun(disease='pneumococcal',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',yearN,monthN, race_recode_new) 
p3$plot1 + facet_wrap(~race_recode_new, scales='free', nrow=1)
```

```{r, fig.width=12, fig.height=3}
p3 <- death_plot_fun(disease='pneumococcal_pneum_no_ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',yearN,monthN, race_recode_new) 
p3$plot1 + facet_wrap(~race_recode_new, scales='free', nrow=1)
```

```{r, fig.width=12, fig.height=3}
p3 <- death_plot_fun(disease='ipd',reg.form='N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6',yearN,monthN, race_recode_new) 
p3$plot1 + facet_wrap(~race_recode_new, scales='free', nrow=1)
```
...
pneumo.deaths.merge <- pneumo.deaths %>%
  filter(!is.na(agec) & !is.na(race_recode_new)) %>%
    group_by(agec, race_recode_new, year) %>%
  summarize(N_deaths=n()) %>%
    ungroup() %>%
  dplyr::select(year,agec, race_recode_new,N_deaths) %>%
  mutate(agec=as.character(agec),
         agec=if_else(agec=='5-24 years','5-24 Years', agec))

pop2.yr <- pop2 %>%
  group_by(agec,race_recode_new, year) %>%
  summarize( pop=mean(pop.interpol))

p3.ds <- pneumo.deaths.merge %>%
  full_join( pop2.yr, by=c('agec','race_recode_new', 'year')) %>%
  filter(year<2020 & !is.na(race_recode_new)) %>% #Pre-padnemic rates
  ungroup() %>%
  mutate(N_deaths = if_else(is.na(N_deaths),0 ,N_deaths),
      inc=N_deaths/pop*100000 ,
         race_recode_new=as.character(race_recode_new)) %>%
  filter(!is.na(race_recode_new)) %>%
  mutate( race_short = if_else(race_recode_new=='American Indian', 'Amer. Ind.',
                               if_else( race_recode_new=='Asian/Pacific Islanders', 'Asian/PI',
                                      race_recode_new)),
           race_recode_new=factor(race_recode_new),
          
          agec = factor(agec, levels=c("Under 5 Years", "5-24 Years",
  ...
  ungroup() %>%
  group_by(race_short, agec) %>%
  summarize(inc=mean(inc)) #average annual incidence
  

ggplot(p3.ds, aes(x=agec, y=(inc), group=race_short,col=race_short, linetype=race_short)) +
  geom_line()+
  theme_classic()+
  ...
  ylab('Deaths/100,000') +
  xlab('') +
    geom_dl(aes(label = race_short), method = "last.qp") +
...
Pneumococcal pneumonia (no IPD)
```{r}
p1_pneumococcal_pneum_no_ipd$ts.plot


p1_pneumococcal_pneum_no_ipd$rr.plot
...
p3 <- death_plot_fun(disease='pneumococcal',year,month, race_recode_new) 
...
mod.mat3 <- as.data.frame(model.matrix(~index+ sin12+cos12 +sin6 +cos6 + race_recode_new, data=ds3))
names(mod.mat3) <- gsub('-','_',names(mod.mat3))
names(mod.mat3) <- gsub(' ','_',names(mod.mat3))
names(mod.mat3) <- gsub('/','_',names(mod.mat3))
#names(mod.mat3) <- gsub('race_recode_new','',names(mod.mat3), fixed=T)

ds3 <- cbind.data.frame(ds3,mod.mat3[,c( grep('race_recode_new', names(mod.mat3)))] )

#Poisson
mod3 <- glm(N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6  + race_recode_newBlack +race_recode_newHispanic + race_recode_newAmerican_Indian + race_recode_newAsian_Pacific_Islanders , 
...
ggplot(data=ds3, aes(x=date, y=N_deaths)) +
  ...
  facet_wrap(~race_recode_new, scales='free')+
...
p3 <- death_plot_fun(disease='pneumococcal',year,month, race_recode_new, agec) 
...
ds3 <- p3$ts %>%
  mutate(date=as.Date(paste(year, month, '01', sep='-')),
         ...
         N_deaths_pre= if_else(year %in% '2020', NA_real_,N_deaths),
         log_offset=log(1)) %>%
  filter( !is.na(agec) & agec %in% c('25-44 years','45-64 years','65-74 years', '75-84 years','85 years and older') & !is.na(race_recode_new) )  
...
p3 <- death_plot_fun(disease='pneumococcal',year,month, j13, agec) 
...
mod.mat3 <- as.data.frame(model.matrix(~index+ sin12+cos12 +sin6 +cos6 + agec +j13, data=ds3))
names(mod.mat3) <- gsub('-','_',names(mod.mat3))
names(mod.mat3) <- gsub(' ','_',names(mod.mat3))
names(mod.mat3) <- gsub('/','_',names(mod.mat3))
#names(mod.mat3) <- gsub('race_recode_new','',names(mod.mat3), fixed=T)
...
#Poisson
mod3 <- glm(N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6  + 
                                      agec45_64_years + agec65_74_years + agec75_84_years +  agec85_years_and_older  + j13   , 
            data=ds3, family='poisson')
...
p1 <-ggplot(data=ds3, aes(x=date, y=N_deaths)) +
  geom_line() +
  theme_classic()+
    geom_ribbon(aes(ymin=pred_lcl, ymax=pred_ucl), alpha=0.2) +
  facet_grid(j13~agec, scales='free')+
      geom_vline(xintercept=as.Date('2020-03-01'), lty=2, col='gray80')+
  coord_cartesian(xlim=c(as.Date('2020-01-01'), as.Date('2021-12-01')))
p1
ggplotly(p1)
...
p3 <- death_plot_fun(disease='pneumococcal',year,month, j13) 
...
mod.mat3 <- as.data.frame(model.matrix(~index+ sin12+cos12 +sin6 +cos6 + j13, data=ds3))
names(mod.mat3) <- gsub('-','_',names(mod.mat3))
names(mod.mat3) <- gsub(' ','_',names(mod.mat3))
names(mod.mat3) <- gsub('/','_',names(mod.mat3))
#names(mod.mat3) <- gsub('race_recode_new','',names(mod.mat3), fixed=T)

ds3 <- cbind.data.frame(ds3 )

#Poisson
mod3 <- glm(N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6  + j13   , 
            data=ds3, family='poisson')
...
p5 <- death_plot_fun(disease='pneumococcal',year,month,agec, education_recode) 
...
#Poisson
mod5 <- glm(N_deaths_pre ~ index+ sin12+cos12 +sin6 +cos6  + 
...
ggplot(aes(x=date, y=N_deaths)) +
  ...
  theme_classic()+
    geom_ribbon(aes(ymin=pred_lcl, ymax=pred_ucl), alpha=0.2) +
  facet_grid(education_recode~agec, scales='free')+
      geom_vline(xintercept=as.Date('2020-03-01'), lty=2, col='gray80') 
...
ggplot(data=ds5, aes(x=date, y=rr_median)) +
  ...
  facet_grid(education_recode~agec)
```
