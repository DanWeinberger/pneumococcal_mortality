...
```{r}
rsv.deaths <-  open_dataset("R:/NCHS_mortality_public/parquet3", format = "parquet")  %>%
  filter(grepl("J121|J210|J205|B974", all_icd)) %>%
 collect() %>%
  mutate( 
    rsv_pneumonia = if_else(grepl("J121", all_icd),1,0),  
...
deaths2019 <- rsv.deaths %>% filter(year==2019) %>% summarize(N=n())
