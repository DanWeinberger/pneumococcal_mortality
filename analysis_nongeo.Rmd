 ...
 collect() %>%
  mutate( 
   a403 = 1*grepl('A403',all_icd),
    ...
    j12_j18 = max(grepl('J12', all_icd),grepl('J13', all_icd),grepl('J14', all_icd),grepl('J15', all_icd),grepl('J16', all_icd),grepl('J17', all_icd),grepl('J18', all_icd)),
...
#why do all pneumo deaths have a J12-J18?
...
p2a$plot1 + facet_grid(j13~agec, scales='free')
```
...
p3 <- death_plot_fun(year,month, j13, agec) 
...
p3 <- death_plot_fun(year,month, j13) 
...
```{r}
all_icd2 <- unique(unlist(str_split(paste(pneumo.deaths$all_icd, collapse='_'),'_')))

pneumococcal_deaths <- pneumo.deaths %>%
  filter(pneumococcal==1) %>%
  mutate(date = as.Date(paste(year,month,'01', sep='-')))

big.mat_full <- pbsapply(all_icd2, function(X) var1 = grepl(X, pneumococcal_deaths$all_icd) , simplify='array')
